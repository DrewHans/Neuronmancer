...
void ui_create() {
    ...
    printf("Before we begin, there are some things you should know. By design, all models created with Neuronmancer will have at least two layers - one input layer at the beginning of the network and one output layer at the end of the network. These layers are not optional, they are required. You can, however, specify the number of hidden layers and you can also determine the number of neurons in each layer of the network, including the input and output layers.\n");
    ...
    for (int i = 1; i < numberOfLayers - 1; i++) {
        ...
        while(inputBuffer[0] != 's' || inputBuffer[0] != 'S' || \
              inputBuffer[0] != 'r' || inputBuffer[0] != 'R' || \
              inputBuffer[0] != 't' || inputBuffer[0] != 'T' ||) {
    ...
    if(biases == NULL) { onMallocError(numberOfNeuronsTotal * sizeof (double)); }
    
    weights = (double*) malloc(numberOfWeightsTotal * sizeof (double)); //malloc allocates a chunk of host memory
    if(weights == NULL) { onMallocError(numberOfWeightsTotal * sizeof (double)); }
    ...
    #endif
    printf("...initializing weights to random double values in range 0.0-1.0 (inclusive)...\n");
    ...
    // get user input for the learningRate and epochs
    learningRate = -1.0; // assign -1.0 to enter loop
    while(learningRate < 0) {
        // get the learning rate
        printf("? (note, 1 is the minimum)\n~");
        fgets(inputBuffer, MAXINPUT, stdin); // read the user's input
        sscanf(inputBuffer, "%d", &numberOfNeuronsPerLayer[0]); // format and dump the user's input
        if(numberOfNeuronsPerLayer[0] < 1) {
            onInvalidInput(myPatience);
    ...
    void saveModel(numberOfLayers, numberOfNeuronsTotal, numberOfWeightsTotal, \
                   numberOfNeuronsPerLayer, numberOfWeightsPerLayer, \
                   firstNeuronIndexPerLayer, firstWeightIndexPerLayer, \
                   weights, biases, learningRate, epochs);
