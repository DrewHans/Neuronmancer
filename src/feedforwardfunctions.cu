...
/*
 * feedforwardWithDevice method - propagate the inputs forward to compute the outputs
...
void feedforwardWithDevice(double* devNeurons, double* devWeights, int numberOfLayers, int* numberOfNeuronsPerLayer, int* numberOfWeightsPerLayer, int* firstNeuronIndexPerLayer, int* firstWeightIndexPerLayer){
    ...
    for (int l = 1; l < numberOfLayers; l++) {
        combinationFunctionKernel<<<numBlocks, threadsPerBlock>>> (devNeurons, devWeights, firstNeuronIndexPerLayer[l], firstNeuronIndexPerLayer[l-1], firstWeightIndexPerLayer[l], numberOfNeuronsPerLayer[l], numberOfNeuronsPerLayer[l-1]);
...
/*
 * feedforwardWithHost method - propagate the inputs forward to compute the outputs
...
void feedforwardWithHost(double* neurons, double* weights, int numberOfLayers, int* neuronsPerLayer, int* weightsPerLayer, int* firstNeuronIndexPerLayer, int* firstWeightIndexPerLayer) {
    ...
    for (int i = 1; i < numberOfLayers; i++) {
        ...
        for (int j = 0; j < neuronsPerLayer[i]; j++) {
            combinationFunction(neurons, weights, firstNeuronIndexPerLayer[i] + j, firstNeuronIndexPerLayer[i-1], firstWeightIndexPerLayer[i] + j, neuronsPerLayer[i - 1]);
