...
void backpropagationUsingHost(float** neuronDeltas, const float* expected, const float* neurons, const float* weights, const float* biases,
    ...
    for (int no = 0; no < numberOfNeuronsInLayer[numberOfLayers - 1]; no++) {
        ...
        (*neuronDeltas)[indexOfFirstOutputNeuron + no] += (quadraticCostDerivative(expected[no], neurons[indexOfFirstOutputNeuron + no])
                * sigmoidDerivative(neurons[indexOfFirstOutputNeuron + no]));
    }
...
void backpropagationUsingDevice(float* devNeuronDeltas, float* devExpected, float* devNeurons, float* devWeights, float* devBiases,
    ...
    for (int l = numberOfLayers - 2; l > 0; l--) {
        ...
        // for each neuron nl in left-layer:
        // calculate the errorSum of all right-layer connections, then use the errorSum to calculate nl's delta value and add it to neuronDeltas[nl's index]
        cudaKernel_CalculateLeftLayerDeltas<<<blocks, threads>>>(devNeuronDeltas, devExpected, devNeurons, devWeights, numberOfNeuronsInLeft, numberOfNeuronsInRight,
...
__global__ static void cudaKernel_CalculateOutputLayerDeltas(float* __restrict__ devNeuronDeltas, const float* __restrict__ devExpected,
    ...
    if (no < numberOfNeuronsInOutput) {
        ...
        devNeuronDeltas[indexOfFirstOutputNeuron + no] += (quadraticCostDerivative(devExpected[no], devNeurons[indexOfFirstOutputNeuron + no])
                * sigmoidDerivative(devNeurons[indexOfFirstOutputNeuron + no]));
    }
...
void updateWeightsUsingDevice(float* devNeuronDeltas, float* devNeurons, float* devWeights, const unsigned int numberOfLayers,
    ...
    for (int l = 1; l < numberOfLayers; l++) {
        ...
        // use the devNeuronDeltas to update every bias in the network
        cudaKernel_updateWeightsBetweenLayers<<<blocks, threads>>>(devNeuronDeltas, devNeurons, devWeights, numberOfNeuronsInLeft, numberOfNeuronsInRight,
                numberOfWeightsBetween, indexOfFirstLeftNeuron, indexOfFirstWeight, learningRate);
