...
void readModelValuesFromDisk(unsigned int* p_numberOfLayers, unsigned int* p_numberOfNeuronsTotal, unsigned int* p_numberOfWeightsTotal,
    ...
    (*p_numberOfNeuronsInLayer) = (unsigned int *) malloc((*p_numberOfLayers) * sizeof(int));
    if ((*p_numberOfNeuronsInLayer) == NULL) {
        onMallocError((*p_numberOfLayers) * sizeof(int));
    }
    ...
    (*p_numberOfWeightsInFrontOfLayer) = (unsigned int *) malloc((*p_numberOfLayers) * sizeof(int));
    ...
    (*p_indexOfFirstNeuronInLayer) = (unsigned int *) malloc((*p_numberOfLayers) * sizeof(int));
    ...
    (*p_indexOfFirstWeightInFrontOfLayer) = (unsigned int *) malloc((*p_numberOfLayers) * sizeof(int));
    ...
    while ((characters = getline(&buffer, &bufsize, thefile)) != -1) {
        ...
        while (token) {
            sscanf(token, "%u", &((*p_numberOfNeuronsInLayer)[layerIndex])); // store token
            token = strtok(NULL, VALUEDELIM); // grab next token from line
            sscanf(token, "%u", &((*p_numberOfWeightsInFrontOfLayer)[layerIndex])); // store token
            token = strtok(NULL, VALUEDELIM); // grab next token from line
            sscanf(token, "%u", &((*p_indexOfFirstNeuronInLayer)[layerIndex])); // store token
            token = strtok(NULL, VALUEDELIM); // grab next token from line
            sscanf(token, "%u", &((*p_indexOfFirstWeightInFrontOfLayer)[layerIndex])); // store token
            token = strtok(NULL, VALUEDELIM); // grab next token from line
        }
    }
...
void readModel(float* p_learningRate, unsigned int* p_epochs, unsigned int* p_numberOfLayers, unsigned int* p_numberOfNeuronsTotal,
    ...
    readModelValuesFromDisk(p_numberOfLayers, p_numberOfNeuronsTotal, p_numberOfWeightsTotal, p_numberOfNeuronsInLayer, p_numberOfWeightsInFrontOfLayer,
