...
// Consider adding __constant__ after __device__ to store array in
// constant memory on the GPU device. If you get "ptxas error : File
// uses too much global constant data" then your device doesn't have
// enough constant memory to store the arrays
...
void cuda_trainNetwork(
    ...
    unsigned int hThreads,
    unsigned int oBlocks,
    unsigned int oThreads);
